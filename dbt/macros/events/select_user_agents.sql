{% macro select_user_agents(from_date=False, to_date=False, materialized=None) %}
    ...
    WITH
        user_agents AS (
            SELECT DISTINCT
                ...
                , {{ get_user_agent_id('user_agent') }} AS user_agent_id
    ...
    SELECT
          ua.user_agent_id AS user_agent_id
    ...
    WHERE
        ...
        {% if is_incremental() %}
        AND (
            SELECT COUNT(s.user_agent_id)
            FROM {{ this }} AS s
            WHERE s.user_agent_id = ua.user_agent_id
        ) = 0
        {% endif %}
